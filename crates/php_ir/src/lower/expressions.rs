...
fn constant_array_matches_expression(
    ...
    let Some(expression) = module.expressions().get(expr_id) else {
        return false;
    };
    let (HirExprKind::Array { elements }, IrConstant::Array(entries)) =
        (expression.kind(), constant)
    else {
        return true;
    };
    elements.len() == entries.len()
        && elements.iter().zip(entries).all(|(element_id, entry)| {
            let Some(element) = module.expressions().get(*element_id) else {
                return false;
            };
            match element.kind() {
                HirExprKind::ArrayPair {
                    ...
                    !unpack
                        && !by_ref
                        && match (key, &entry.key) {
                            (Some(key), Some(constant)) => {
                                constant_array_matches_expression(module, *key, constant)
                            }
                            (None, None) => true,
                            _ => false,
                        }
                        && value.is_some_and(|value| {
                            constant_array_matches_expression(module, value, &entry.value)
                        })
                }
                _ => {
                    entry.key.is_none()
                        && constant_array_matches_expression(module, *element_id, &entry.value)
                }
            }
        })
}
...
impl LoweringContext<'_> {
        ...
        module
            ...
            .and_then(|(_, const_expr)| {
                ...
                constant_from_expr_with_runtime_constants(
                    ...
                    const_expr.expr_id(),
                    rt_inputs(
                        (named_constants, current_class),
                        (class_constants, class_parents),
                    ),
                    &|expr| self.lower_magic_constant_expr(module, expr, current_class),
                    &mut Vec::new(),
                )
            })
            .or_else(|| {
    ...
    pub(super) fn lower_const_expr_magic_constant(
        ...
        let module = self
            ...
            .module(self.frontend.module().module_id())?;
        let const_expr = module.const_exprs().get(const_expr_id)?;
        self.lower_magic_constant_expr(module, const_expr.expr_id(), current_class_display)
    }

    fn lower_magic_constant_expr(
        &self,
        module: &HirModule,
        expr_id: ExprId,
        current_class_display: Option<&str>,
    ...
    pub(super) fn lower_const_expr_value(
        ...
        constant_from_expr_with_runtime_constants(
            ...
            &|expr| {
                self.lower_magic_constant_expr(
                    module,
                    expr,
                    current_class_display.or(current_class),
                )
            },
            &mut visiting,
        )
            ...
            HirExprKind::Name { resolution } => {
                ...
                let instruction = builder.emit(
                    ...
                    InstructionKind::FetchConst {
                        ...
                        fallback: resolution
                            .fallback()
                            .map(|fallback| fallback.trim_start_matches('\\').to_string()),
                    },
                    span,
                );
                self.add_expr_source_map(builder, function, block, instruction, expr, span);
    ...
    /// Returns locals that may bind by reference without materializing a value
    /// register and pinning its copy-on-write handle.
    ///
    /// Only unconditional same-unit callees qualify: table membership proves
    /// the stable signature. The positional mapping must supply every required
    /// parameter so the arity precheck never observes a placeholder.
...
pub(super) fn class_constant_fetch_class_name(
    class_name: String,
