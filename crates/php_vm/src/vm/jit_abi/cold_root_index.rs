...
pub(super) fn baseline_shared_array_storage_contains_object(
    address: usize,
    object_id: u64,
) -> bool {
    php_runtime::api::PhpArray::clone_from_native_storage_refcount(address)
        .is_some_and(|array| values_contain_object(array.iter().map(|(_, value)| value), object_id))
}

pub(super) fn rooted_membership_may_change(previous: &Value, replacement: &Value) -> bool {
...
impl<'a> super::NativeRequestColdState<'a> {
    pub(super) fn mark_roots_dirty(&mut self, reason: RootMutationReason) {
        self.cross_unit_stable_values.clear();
        self.root_index.mark_dirty(reason);
    }

    pub(super) fn mark_rooted_container_dirty(&mut self, value: &Value) {
        self.root_index
            .mark_dirty(RootMutationReason::RootedContainer);
        self.root_index.refresh_container(value);
    }

    pub(super) fn value_has_native_destructor(&self, value: &Value) -> bool {
    ...
    pub(super) fn synchronize_destructor_root_change(
        &mut self,
        previous: &Value,
        replacement: &Value,
    ) {
    ...
    pub(super) fn add_rooted_nested_container(&mut self, parent: &Value, child: &Value) {
        if self.root_index.is_dirty() || self.root_index.contains_container(parent) {
            self.root_index.add_nested_container(parent, child);
        }
    }

    pub(super) fn request_root_values(&self) -> Vec<Value> {
        let mut roots = self
            .baseline_values
            .static_property_transfer
            .values()
            .chain(self.baseline_values.cold_dynamic_constants.values())
            .chain(self.baseline_values.inherited_globals.values())
            .cloned()
            .collect::<Vec<_>>();
        roots.extend(
        ...
        roots.push(Value::Reference(
            self.baseline_values.session_global.clone(),
        ));
        roots.extend(self.baseline_values.pending_throwable.iter().cloned());
        roots.extend(
            self.baseline_values
                .enum_cases
                .values()
    ...
    pub(super) fn synchronize_request_roots(&mut self) {
    ...
    pub(super) fn consume_native_root_mutation(&mut self) {
        if *self.native_root_mutation_pending == 0 {
            return;
        }
        *self.native_root_mutation_pending = 0;
        // A generated store can add a unit-local literal to a container that
        // was stabilized before the store. Invalidate the graph cache before
        // the next cross-unit traversal; root reachability and cross-unit
        // representation are both affected by this mutation boundary.
        self.cross_unit_stable_values.clear();
        self.root_index
            .mark_dirty(RootMutationReason::RootedContainer);
    }

    pub(super) fn finalize_replaced_value(&mut self, previous: Value) -> Result<(), String> {
        if let Value::Object(object) = previous {
            ...
            {
                return Err(format!(
                    "final owner of {} must enter the generated release spine",
                    object.class_name()
                ));
            }
        }
...
#[cfg(test)]
mod tests {
    //! Cold request-root indexing for materialized Rust boundary values.
