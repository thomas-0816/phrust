...
impl<'a> NativeRequestColdState<'a> {
    ...
    pub(super) fn duplicate_registered_callback(&mut self, encoded: i64) -> Result<i64, String> {
        self.duplicate_authoritative_dereferenced_native_value(encoded)?
            .ok_or_else(|| "registered callback crossed from baseline storage".to_owned())
    }

    pub(super) fn duplicate_registered_callback_argument(
        &mut self,
        encoded: i64,
    ) -> Result<i64, String> {
        self.duplicate_authoritative_native_value(encoded)?
            .ok_or_else(|| "registered callback argument crossed from baseline storage".to_owned())
    }

    pub(super) fn native_registered_callbacks_equal(&self, left: i64, right: i64) -> bool {
        ...
        match (
            self.native_encoded_value_kind(left),
            self.native_encoded_value_kind(right),
        ) {
            (Some(NativeEncodedValueKind::String), Some(NativeEncodedValueKind::String)) => {
                self.native_string_bytes(left) == self.native_string_bytes(right)
            }
            (Some(NativeEncodedValueKind::Object), Some(NativeEncodedValueKind::Object)) => {
                self.native_query_object(left).map(|object| object.id())
                    == self.native_query_object(right).map(|object| object.id())
            }
            (Some(NativeEncodedValueKind::Array), Some(NativeEncodedValueKind::Array)) => {
                let Some((left_target, left_method)) =
                    baseline_native_builtins::native_callable_array_parts(self, left)
                else {
                    return false;
                };
                let Some((right_target, right_method)) =
                    baseline_native_builtins::native_callable_array_parts(self, right)
                else {
                    return false;
                };
                self.native_registered_callbacks_equal(left_target, right_target)
                    && self.native_registered_callbacks_equal(left_method, right_method)
            }
            (Some(NativeEncodedValueKind::Callable), Some(NativeEncodedValueKind::Callable)) => {
                let Some(left_index) = Self::direct_value_index(left) else {
                    return false;
                };
                let Some(right_index) = Self::direct_value_index(right) else {
                    return false;
                };
                let Some(left) = self.direct_prepared_callable_view(left_index) else {
                    return false;
                };
                let Some(right) = self.direct_prepared_callable_view(right_index) else {
                    return false;
                };
                ...
                match left.kind {
                    ...
                    php_jit::JIT_NATIVE_CALLABLE_KIND_BOUND_OBJECT_METHOD => {
                        self.native_registered_callbacks_equal(left.receiver, right.receiver)
                            && self.native_callable_bytes_equal(
                                left.method_bytes,
                                left.method_length,
                                right.method_bytes,
                                right.method_length,
                    ...
                    _ => self.native_callable_bytes_equal(
                        left.name_bytes,
                        left.name_length,
                        right.name_bytes,
                        right.name_length,
    ...
    #[allow(unsafe_code)]
    fn native_callable_bytes_equal(
        &self,
        left: u64,
        left_length: u32,
        right: u64,
        ...
        let Ok(length) = usize::try_from(left_length) else {
            return false;
        };
        let (Ok(left), Ok(right)) = (usize::try_from(left), usize::try_from(right)) else {
            return false;
        };
        // SAFETY: both byte ranges belong to live prepared callable owners,
        // validated by `direct_prepared_callable_view` above.
        unsafe {
            std::slice::from_raw_parts(left as *const u8, length)
                == std::slice::from_raw_parts(right as *const u8, length)
        }
    }
            ...
            for callback in transfer.shutdown_callbacks {
                ...
                native
                    ...
                    .push(NativeRegisteredShutdownCallback {
                        callable,
                        arguments,
                        source: NativeRegisteredCallbackSource::Cold(callback.source),
                        transient_export: false,
                    });
            }
    ...
    fn decode_registered_callback_state(
        ...
        for callback in &state.shutdown_callbacks {
            let source = match &callback.source {
                NativeRegisteredCallbackSource::Cold(source) => source.clone(),
                NativeRegisteredCallbackSource::NativeContinuation {
                    function,
                    continuation,
                } => self
                    ...
                    .ok_or_else(|| {
                        "registered shutdown callback source is unavailable".to_owned()
                    })?,
            };
    ...
    pub(super) fn restore_registered_callback_transfer(
        &mut self,
        transfer: NativeRegisteredCallbackTransfer,
    ) -> Result<(), String> {
        let native = self.encode_registered_callback_transfer(transfer)?;
        let previous = std::mem::replace(&mut self.registered_callbacks, native);
        self.release_registered_callback_state(previous)?;
        self.mark_roots_dirty(RootMutationReason::CallbackOrHandler);
        Ok(())
    }
...
impl<'a> NativeRequestColdState<'a> {
    ...
    pub(super) fn unset_native_global_dimension(
        &mut self,
        key: &php_runtime::api::ArrayKey,
    ) -> Result<bool, String> {
        self.ensure_native_global_references();
            ...
            .cloned()
        {
            ...
            {
                // The baseline continuation still carries the detached
                // top-level local through its final SSA release. Transfer the
                // symbol-table owner's native reference to that continuation
                // before invalidation removes the global publication.
                self.retain(encoded)?;
            }
