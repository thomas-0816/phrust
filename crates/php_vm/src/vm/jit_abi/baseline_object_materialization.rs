...
pub(super) fn new_native_object(
    context: &NativeRequestColdState<'_>,
    owner_unit: Option<usize>,
    class: &php_ir::module::ClassEntry,
...
pub(super) fn native_prepare_runtime_class_constants(
...
pub(super) fn encode_native_enum_case(
    context: &mut NativeRequestColdState<'_>,
    class: &php_ir::module::ClassEntry,
    case: &php_ir::module::ClassEnumCaseEntry,
) -> Result<i64, String> {
    let key = (class.name.clone(), case.name.clone());
    if let Some(object) = context.baseline_values.enum_cases.get(&key).cloned() {
