//! Baseline/reflection object materialization.
//!
//! This module is the explicit cold boundary for PHP metadata objects,
//! reflection graphs, and extension-style object compatibility. Optimizing
//! artifacts use native object slots and cannot import this module.
    ...
    match &instruction.kind {
        ...
        php_ir::InstructionKind::NewObject {
            ...
            if let Some(array) = arguments.first()
                && let Ok(Value::Array(array)) = context.decode_baseline_value(*array)
            {
            ...
            || class_name.eq_ignore_ascii_case("ReflectionEnumBackedCase") =>
        {
            let result = (|| -> Result<i64, String> {
                if class_name.eq_ignore_ascii_case("ReflectionFunction") {
                    ...
                    let value = match context.decode_baseline_value(*argument)? {
                ...
                if class_name.eq_ignore_ascii_case("ReflectionMethod") {
                    ...
                    let owner = match context.decode_baseline_value(arguments[0])? {
                        Value::Reference(reference) => reference.get(),
                        value => value,
                    };
                    ...
                    let method = match context.decode_baseline_value(arguments[1])? {
                        Value::Reference(reference) => reference.get(),
                        value => value,
                    };
                ...
                if class_name.eq_ignore_ascii_case("ReflectionProperty") {
                    ...
                    let member = match context.decode_baseline_value(arguments[1])? {
                        Value::Reference(reference) => reference.get(),
                        value => value,
                    };
                ...
                match method.to_ascii_lowercase().as_str() {
                    ...
                    "getbackingtype" => {
                        ...
                        let type_name = context
                            ...
                            .unwrap_or("mixed");
                        context.encode_baseline_value(native_reflection_type(&match type_name {
                    ...
                    "getmethods" | "getmethod" => {
                        ...
                        if requested.is_some() {
                            context.encode_baseline_value(
                                methods.into_iter().next().unwrap_or(Value::Null),
                            )
                        } else {
                    ...
                    "getproperties" | "getproperty" => {
                        ...
                        if requested.is_some() {
                            context.encode_baseline_value(
                                properties.into_iter().next().unwrap_or(Value::Null),
                            )
                        } else {
                            context.encode_native_array_owner(
                    ...
                    "getconstants" | "getconstant" | "getreflectionconstant" => {
                        ...
                        if method.eq_ignore_ascii_case("getConstant") {
                            return context.encode_baseline_value(value);
                        }
                        let entry =
