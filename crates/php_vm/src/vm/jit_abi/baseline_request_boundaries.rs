    ...
    pub(in crate::vm) fn run_shutdown_callbacks(&mut self) -> Result<(), String> {
        ...
        while !self.registered_callbacks.shutdown_callbacks.is_empty() {
            ...
            let source = match source {
                NativeRegisteredCallbackSource::Cold(source) => source,
            ...
            let result =
                invoke_native_encoded_callable_value_from(self, &encoded, &source, None, None)
        ...
        let result = loop {
            ...
            if let Err(error) = self.run_object_destructor(object) {
                break Err(error);
            }
        };
