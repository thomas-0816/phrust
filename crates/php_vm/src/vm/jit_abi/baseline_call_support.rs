//! Baseline-native call binding and PHP `Value` semantics.
//!
//! Exact and optimizing artifacts exchange authoritative native encodings
//! through fixed ABIs and cannot name these Rust helpers. This code is entered
//! only after one explicit baseline continuation.

use super::*;
use php_runtime::api::PhpString;
use php_runtime::api::Value;

/// Structured control leaving one baseline-native PHP call.
///
/// Successful calls still carry the authoritative native encoding unchanged.
/// This compatibility type is deliberately defined in the baseline module:
/// exact optimizing handlers publish `JitNativeControlResult` directly and
/// cannot parse or serialize the legacy `E_PHP_*` envelope.
#[derive(Debug)]
...
pub(super) fn encode_native_magic_call_arguments_array(
...
fn expand_baseline_user_call_arguments(
    ...
    for (argument, value) in metadata.iter().zip(arguments) {
        ...
        if let Some((entry_start, entry_count)) = context.direct_array_entry_range(*value) {
...
fn bind_baseline_by_value_parameter(
    ...
    Ok(context.encode_baseline_value(value)?)
}
    ...
    let binding = (|| -> Result<request_state::NativeTraceArguments, NativeCallControl> {
        ...
        for argument in supplied {
            if let Some(name) = argument.name.clone() {
                    ...
                    .position(|parameter| parameter.name == name)
                {
                    ...
                    if !variadic_names.insert(name.clone()) {
                        return Err(NativeCallControl::throw(
                            "Error",
                            format!("Named parameter ${name} overwrites previous argument"),
                        ));
        ...
        for (index, parameter) in target_params.iter().enumerate() {
            ...
            if let Some(argument) = assigned[index].as_ref() {
                ...
                let required = target_params
                    .iter()
                    .filter(|parameter| parameter.required)
                    .count();
                return Err(NativeCallControl::ArgumentCount {
                    function: target_name.to_owned(),
                    passed: supplied_count,
                    required,
                    target_span: target_metadata.span,
                });
            }
        }
...
fn invoke_native_with_owned_bound_arguments(
    ...
    let borrowed_reference_arguments = metadata.as_ref().map_or_else(Vec::new, |metadata| {
        let leading = metadata.capture_count
            + usize::from(metadata.instance_method)
            + usize::from(metadata.implicit_closure_this);
        metadata
            .params
            .iter()
            .enumerate()
            .filter(|(_, parameter)| parameter.by_ref && !parameter.variadic)
            .filter_map(|(index, _)| bound.get(leading + index).copied())
            .collect::<Vec<_>>()
    });
    // Mutable by-value parameter locals retain and release their own frame
    // owner. Fixed by-reference locals deliberately borrow the stable
    // reference identity instead; the binder's independent owner therefore
    // remains the caller's responsibility after the synchronous invocation.
    ...
    let mut release_error = None;
    for reference in borrowed_reference_arguments {
        if let Err(error) = context.release_if_live(reference) {
            release_error.get_or_insert(error);
        }
    }
    match (result, release_error) {
...
pub(super) fn external_function_is_generator(
...
pub(super) fn bind_native_builtin_arguments<'a>(
    ...
    if baseline_native_builtins::native_builtin_is_unavailable_target_function(name)
...
pub(super) fn invoke_native_closure_payload(
    context: &mut NativeRequestColdState<'_>,
    ...
    if let Some(captures) = prepared_captures {
        ...
        for capture in &closure.captures {
                ...
                && let Some(object) = &closure.bound_this
            {
                ...
                context.encode_baseline_value(capture.value().cloned().unwrap_or(Value::Null))?
            };
            closure_arguments.push(encoded);
            temporary_owners.push(encoded);
        }
...
pub(super) fn execute_native_dynamic_callable(
    ...
    match context.native_encoded_value_kind(direct_callee) {
        ...
        Some(NativeEncodedValueKind::Array) => {
            if let Some((entry_start, entry_count)) =
                context.direct_array_entry_range(direct_callee)
            {
                if entry_count != 2 {
            ...
            match method.to_ascii_lowercase().as_str() {
                ...
                "current" | "key" => {
                    ...
                    value.map_or_else(
                        ...
                        |value| {
                            context
                                .duplicate_direct_generator_value(value)
                                .map_err(Into::into)
                        },
                    )
                }
                ...
                _ => Err(format!("Call to undefined method Generator::{method}()").into()),
            }
        })();
        return Some(result);
    }
    if context.native_encoded_value_kind(*receiver) != Some(NativeEncodedValueKind::Generator) {
        return None;
    }
    ...
    let result = (|| -> NativeCallResult {
        ...
        match method.to_ascii_lowercase().as_str() {
            "rewind" => {
                ...
                context
                    .encode_baseline_value(Value::Null)
                    .map_err(Into::into)
            }
            "valid" => {
                ensure_started(context)?;
                context
                    .encode_baseline_value(Value::Bool(
            ...
            "current" => {
                ensure_started(context)?;
                context
                    .encode_baseline_value(generator.current_value().unwrap_or(Value::Null))
                    .map_err(Into::into)
            }
            "key" => {
                ensure_started(context)?;
                context
                    .encode_baseline_value(generator.current_key().unwrap_or(Value::Null))
                    .map_err(Into::into)
            }
            "next" => {
                ensure_started(context)?;
            ...
            "send" => {
                ...
                context
                    .encode_baseline_value(next.map_or(Value::Null, |(_, value)| value))
                    .map_err(Into::into)
            }
            "throw" => {
                ensure_started(context)?;
            ...
            "getreturn" => {
                ...
                context
                    .encode_baseline_value(generator.return_value().unwrap_or(Value::Null))
                    .map_err(Into::into)
            }
            _ => Err(format!("Call to undefined method Generator::{method}()").into()),
        }
...
pub(super) fn finish_native_fiber_outcome(
    ...
    match outcome {
        ...
        php_jit::JitI64InvokeOutcome::SideExit { status, state, .. } => {
            ...
            Err(format!("native fiber returned status {status}{continuation}{diagnostic}").into())
        }
    }
}
...
pub(super) fn execute_native_fiber_suspend(
    ...
    if context.active_fiber.is_none() {
        return Some(Err(NativeCallControl::throw(
            "FiberError",
    ...
    Some(Err(NativeCallControl::SuspendFiber { state: None }))
}
        ...
        match method.to_ascii_lowercase().as_str() {
            ...
            "getreturn" => {
                ...
                if fiber_state != php_runtime::api::FiberState::Terminated {
                    ...
                    return Err(NativeCallControl::throw(
                        "FiberError",
                        format!("Cannot get fiber return value: The fiber has not {state}"),
                    ));
                }
                context
            ...
            "start" => {
                ...
                let Some((closure, implicit_this, captures)) =
                    ...
                    return Err("Fiber callback must resolve to a native closure".into());
                };
                let function = php_ir::FunctionId::new(closure.function);
                let owner_unit = closure.context.owner_unit;
                let target = NativeExecutionTarget {
                ...
                let outcome = context.run_in_native_execution_target(
                    &target,
                    |context| -> NativeCallResult {
                        let arguments = arguments.take().ok_or_else(|| {
                            NativeCallControl::from(
                                "Fiber arguments were already consumed before native entry",
                            )
                        })?;
                        let runtime = context.native_runtime_ptr();
                        ...
                        let (handle, outcome) = match resume_native_optimizing_exit_with_artifact(
                            ...
                            Ok((Some(handle), outcome)) => (handle, outcome),
                            Ok((None, _)) => {
                                for argument in arguments {
                                    context.release_if_live(argument)?;
                                }
                                return Err(
                                    "native Fiber invocation lost its active artifact".into()
                                );
                            }
                            Err(error) => {
                                for argument in arguments {
                                ...
                                return Err(
                                    format!("native fiber invocation failed: {error:?}").into()
                                );
                            }
                        };
            ...
            _ => Err(format!("Call to undefined method Fiber::{method}()").into()),
        }
    })();
    Some(result)
}
...
/// Invokes the request's authoritative native autoload roots without
/// reconstructing callback or argument `Value`s. The snapshot owns each
/// callback independently so an autoloader may unregister itself while it is
/// running without invalidating the active iteration.
pub(super) fn invoke_registered_autoload_callbacks_until(
    context: &mut NativeRequestColdState<'_>,
    class_name: &[u8],
    source: &php_ir::Instruction,
    mut loaded: impl FnMut(&mut NativeRequestColdState<'_>) -> bool,
) -> Result<(), String> {
    ...
    for callback in callbacks {
        if let Err(error) = context.retain(callback) {
            for callback in retained {
                let _ = context.release_if_live(callback);
            }
            return Err(error);
        }
        retained.push(callback);
    }
    let class_name = match context.encode_direct_string_bytes(class_name) {
    ...
    let result = (|| {
        for callback in retained.iter().copied() {
    ...
    let mut release_error = context.release_if_live(class_name).err();
    for callback in retained {
        if let Err(error) = context.release_if_live(callback) {
            release_error.get_or_insert(error);
        }
    }
...
/// Gives an external native frame one owner for every argument without
/// decoding or rebuilding ordinary values. Unit-local literals are published
/// into request-wide native storage and direct ownership graphs are stabilized
/// in place before their handles cross the unit boundary.
pub(super) fn transfer_native_external_arguments(
    context: &mut NativeRequestColdState<'_>,
    arguments: &[i64],
    ...
    arguments
        ...
        .map(|argument| {
                ...
                )
            {
                ...
                // the complete literal directly into request-wide native
                // storage before switching the active unit.
                return context.stabilize_active_unit_constant(constant);
            }
            if let Some(index) = NativeRequestColdState::direct_value_index(encoded) {
                let slot = context
                    ...
                    .ok_or_else(|| format!("direct native argument {index} is dead"))?;
                if slot.kind == php_jit::JIT_NATIVE_VALUE_VIEW_GLOBALS_PROXY {
                    // `$GLOBALS` is an explicit baseline-only compatibility
                    // projection, never an ordinary optimizing value.
                    let globals = context.materialize_native_globals_array()?;
                    return context.encode_baseline_value(globals);
                }
                if matches!(
                    slot.kind,
                    ...
                    return Err(format!(
                        "native iterator {index} cannot cross a function-call boundary"
                    ));
                }
            ...
            if let Some(index) = php_jit::jit_decode_runtime_value(encoded) {
                return Err(format!(
                    "native call argument {index} is outside the authoritative direct slot plane"
                ));
            }
            // Immediate integers and the four unit-independent constants are
            // owner-free native values and pass through unchanged.
            Ok(encoded)
        })
...
pub(super) fn invoke_native_method(
    context: &mut NativeRequestColdState<'_>,
    function: php_ir::FunctionId,
    arguments: &[i64],
) -> NativeCallResult {
...
fn invoke_native_method_with_prepared_trace_arguments(
    ...
    let object = if instance_method {
        // Every admitted method receiver is an authoritative native object
        // handle (possibly behind a direct native reference). Backtraces
        // borrow that owner directly; there is no compatibility decode or
        // object-graph demotion on the common call path.
        arguments
            .first()
    ...
    context.publish_active_call_argument_view();
    ...
    context.restore_materialized_direct_references()?;
    let execution_target = NativeExecutionTarget {
        unit: context.current_dynamic_unit,
        function,
        called_class: called_class.clone(),
        scope_class: metadata
            .as_ref()
            .and_then(|metadata| metadata.trace_class.clone()),
    };
    let execution_scope = context.register_native_execution_scope(execution_target.scope())?;
    let previous_execution_scope =
        std::mem::replace(&mut context.current_native_execution_scope, execution_scope);
    let push_scope_class = execution_target
        .scope_class
        .as_ref()
        .is_some_and(|scope_class| {
            context.lexical_scope_classes.last().map(String::as_str) != Some(scope_class.as_ref())
        });
    if push_scope_class {
        context.lexical_scope_classes.push(
            execution_target
                .scope_class
                .as_ref()
                .expect("method scope class was classified above")
                .to_string(),
        );
    }
    // Publish the complete same-unit method scope before entering generated
    // code. Exact closure allocation reads this identity directly from the
    // fast state; pushing only the legacy called-class stack left lexical
    // `self`/`parent` context behind while late-static `static` happened to
    // keep working.
    ...
    let outcome = resume_native_optimizing_exit(context, handle.clone(), outcome);
    let restore_result = context.restore_materialized_direct_references();
    if push_scope_class {
        context.lexical_scope_classes.pop();
    }
    context.current_native_execution_scope = previous_execution_scope;
    let _runtime_view = activate_native_context(context);
    restore_result?;
    ...
    match outcome {
        Ok(php_jit::JitI64InvokeOutcome::Returned(value)) => {
            ...
            if returns_by_ref {
                ...
                // The returned encoding already owns its native payload.
                // Move that owner directly into a reference descriptor instead
                // of decoding a Rust `Value` and promoting it again.
                return Ok(context.encode_direct_reference_payload_owned(value)?);
            }
            Ok(value)
        }
        ...
        Ok(php_jit::JitI64InvokeOutcome::SideExit {
            ...
            append_native_throwable_frame(
                context,
                value,
                function_name,
                &completed_frame.arguments,
            )
            .map_err(NativeCallControl::RuntimeError)?;
            let throwable = context.decode_baseline_value(value).map_err(|error| {
                let continuation = context
                    .instruction_for_continuation(state.function_id, state.continuation_id)
                    .map(|instruction| format!(" at {:?}", instruction.kind))
                    .unwrap_or_else(|| {
            ...
            context.baseline_values.pending_throwable = Some(throwable);
