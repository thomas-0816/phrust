...
fn cli_usage_diagnostic_from_message(message: &str) -> php_diagnostics::DiagnosticEnvelope {
        ...
        .and_then(|rest| rest.strip_suffix('`'))
    {
        (
            ...
            Some(command),
            Some(
                "compile, dump-ir, dump-bytecode-patterns, dump-rule-selection, dump-dependency-units, dump-baseline-native-stencil, dump-mid-tier-plan, dump-cranelift-clif, run, report, compare",
            ),
            "run php-vm --help",
        )
...
fn is_known_native_builtin_call(dense: &DenseBytecodeUnit, operands: &DenseOperands) -> bool {
...
fn classify_mid_tier_instruction(
    ...
    match opcode {
        ...
        DenseOpcode::CallFunction if is_known_native_builtin_call(dense, operands) => {
            push_unique(
                &mut plan.candidate_optimizations,
...
mod tests {
    ...
    fn run_default_collects_managed_fast_path_counters() {
        ...
        assert_eq!(counters["native_executions"], 0);
        assert!(
