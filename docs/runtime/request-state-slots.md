...
The VM's request-local `NativeExecutionContext` owns one `BuiltinRequestState`
and one `NativeRegisteredExtensionRequestState` per execution request. Native
builtin dispatch borrows both typed owners into a `BuiltinContext`; include
children temporarily take the owners and return them to the parent, so one PHP
request observes one continuous extension state. The owned context constructor
exists for isolated builtin tests, but its enum representation is exclusive: a
context is either the owner or a borrower and never stores both.
