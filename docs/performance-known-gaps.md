...
| PERF-GAP-JIT-NATIVE-EXECUTION | JIT | `docs/adr/0076-cranelift-jit-experiment.md` accepts only an experiment. `crates/php_jit` and `jit-smoke` cover the API skeleton, conservative eligibility analyzer, handle-based ABI, optional Cranelift IR lowering, guarded safe int-leaf native-entry experiments, tiering/stats, safety-audit coverage, and optional precondition review. Default builds or missing runtime preconditions stay on managed VM paths. | A production native execution claim would be unsafe without W^X, executable-memory ownership, native entry/exit ABI proof, and native execution smoke gates. | future runtime+ native-code follow-up; keep experimental native-entry support separate from production JIT/Opcache claims |
