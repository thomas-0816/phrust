...
| `objects.classes` | 200 PASS | 166 PASS / 34 FAIL | backlog |
...
| --- | --- | --- |
| remaining fatal/exception output parity | class-not-found, iterator exception, and DateTimeInterface fatal output cases | `php_vm` and `php_runtime::error_output` |
...
| serialization and stringification parity | `serialize_001`, `tostring_001` | `php_runtime::object`, standard object formatting |
| class constants, dynamic lookup, autoload, and Reflection catch types | class constant, autoload, ReflectionException fixtures | `php_semantics`, `php_runtime::object`, `php_vm` |
...
| `E_PHP_RUNTIME_UNSUPPORTED_THROWABLE_HIERARCHY` | Narrowed PHP-compatible CLI fatal output by removing duplicate structured stderr when a fatal line is rendered; full stack/object parity remains open. |
...
The highest-risk remaining core blockers are static/property reference lvalues,
iterator/destructor ordering, serialization hooks, class constant inheritance,
eval declaration merging, autoload/Reflection catch behavior, and remaining
...
- Closeout module dashboard: `closure.core` 33/33 PASS, `zend.basic` 10/10 PASS, `operators.conversions` 5/5 PASS, `arrays.references` 6/6 PASS, `zend.functions` 29/29 PASS, and `objects.classes` remains 166 PASS / 34 non-green against a 200 PASS reference run.
