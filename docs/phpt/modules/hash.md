...
- Selected fixtures:
  ...
  - selected `ext/hash/tests/*.phpt` rows for adler32, CRC/FNV/JOOAT, md5,
    RIPEMD, sha1, SHA-2/SHA3, HMAC-md5, file hashing, HKDF, PBKDF2, and stream
    updates
...
`sha3-256`, `sha3-384`, `sha3-512`, `ripemd128`, `ripemd160`, `ripemd256`,
and `ripemd320`; digest-only coverage also includes `adler32`, `crc32`,
...
file digest output, upstream adler32/CRC/FNV/JOOAT/md5/RIPEMD/sha1/SHA-2/SHA3
digest vectors, HMAC-md5, upstream file hashing, `hash_update_file`,
...
`md2`, `md4`, `xxh*`, `haval*`, `tiger*`, `snefru*`, `gost*`, and
`whirlpool` are still unsupported. HashContext magic
...
After adding Adler-32, CRC32/CRC32C, FNV, JOAAT, SHA3, and RIPEMD, the
selected manifest contains 31 green rows.
