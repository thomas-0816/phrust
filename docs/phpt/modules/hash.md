...
- Selected fixtures:
  ...
  - selected `ext/hash/tests/*.phpt` rows for adler32, CRC/FNV/JOOAT, md5, sha1,
    SHA-2/SHA3, HMAC-md5, file hashing, HKDF, PBKDF2, and stream updates
...
`sha3-256`, `sha3-384`, and `sha3-512`; digest-only coverage also includes
...
file digest output, upstream adler32/CRC/FNV/JOOAT/md5/sha1/SHA-2/SHA3 digest
...
`md2`, `md4`, `xxh*`, `ripemd*`, `haval*`, `tiger*`, `snefru*`, `gost*`,
and `whirlpool` are still unsupported. HashContext magic
...
After adding Adler-32, CRC32/CRC32C, FNV, JOAAT, and SHA3, the selected
manifest contains 27 green rows.
