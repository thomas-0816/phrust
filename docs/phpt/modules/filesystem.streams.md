...
- Current target counts: 43 PASS, 10 SKIP, 0 FAIL, 0 BORK from 53 selected
  module fixtures
...
- include_path
- include/require
- selected upstream local file, stat, temp, glob, readfile, `copy`,
  `realpath`, `stream_get_contents`, and `stream_copy_to_stream` behavior
- selected upstream `basename`, `dirname`, and `pathinfo` behavior
- selected upstream `stream_context_set_options` behavior
- selected upstream `chown`/`chgrp` missing-path warning behavior
...
- `ext/standard/tests/streams/stream_context_set_options_basic.phpt`
...
- Broader `file_get_contents`/`readfile` argument diagnostics, stat metadata
  helpers beyond the selected ownership warning rows, stream metadata shape,
  symlink/link helpers, and warning text parity remain outside this selected
  gate.

## Next Step

The selected gate is closed at 53 selected fixtures with 43 PASS and 10
platform SKIP outcomes on target.
...
The selected gate closed with the focused `filesystem.streams` harness at 53
selected fixtures, with 43 PASS and 10 platform SKIP outcomes on target. The
...
lookup, missing require warning/fatal output, selected path helpers, selected
copy/realpath/stream-copy fixtures, and selected ownership warning rows.

Validation:

- `nix develop -c just verify-runtime`: PASS
- `nix develop -c just verify-stdlib`: PASS
- `PHPT_REUSE_LAST=0 PHPT_DEV_REUSE_TARGET_PASS=0 nix develop -c just phpt-dev-module MODULE=filesystem.streams`: reference 43 PASS / 10 SKIP, target 43 PASS / 10 SKIP
...
## Ownership Metadata Report

The selected upstream ownership slice now covers PHP-compatible local
`chown`/`chgrp` function registration and missing-path warning text. The
runtime resolves paths through the existing request-local cwd and filesystem
capability checks, uses Unix `chown(2)` for local numeric/user/group ownership
updates when the platform allows it, and preserves explicit capability failure
as `false`.

Validation:

- `nix develop -c cargo test -p php_runtime chown_and_chgrp_warn_for_missing_paths --no-fail-fast`: PASS, 1 test
- `nix develop -c cargo test -p php_std standard_registry_tracks_stdlib_path_and_stat_functions --no-fail-fast`: PASS, 1 test
- `PHP_SRC_DIR=$PHP_SRC_DIR REFERENCE_PHP=$REFERENCE_PHP PHPT_REUSE_LAST=0 PHPT_DEV_REUSE_TARGET_PASS=0 PHPT_TIMEOUT_SECONDS=10 nix develop -c just phpt-module-target MODULE=filesystem.streams FILE=ext/standard/tests/file/chown.phpt`: PASS, 1 target PHPT
- `PHP_SRC_DIR=$PHP_SRC_DIR REFERENCE_PHP=$REFERENCE_PHP PHPT_REUSE_LAST=0 PHPT_DEV_REUSE_TARGET_PASS=0 PHPT_TIMEOUT_SECONDS=10 nix develop -c just phpt-module-target MODULE=filesystem.streams FILE=ext/standard/tests/file/chgrp.phpt`: PASS, 1 target PHPT

Blocker report: no non-green outcomes remain in the promoted local ownership
warning subset. Symlink ownership helpers (`lchown`, `lchgrp`) and user stream
metadata hooks remain outside this selected gate.

## Stream Context Options Report

The selected upstream stream-context slice now covers
`stream_context_set_options` with nested wrapper options and
`stream_context_get_options` readback. The runtime implementation shares the
existing nested option mutation path with the two-argument
`stream_context_set_option` form while exposing the PHP 8.5
`stream_context_set_options` API as its own builtin and stdlib descriptor.

Validation:

- `nix develop -c cargo test -p php_runtime stream_context_set_options_mutates_nested_options --no-fail-fast`: PASS, 1 test
- `nix develop -c cargo test -p php_std standard_registry_tracks_stdlib_stream_context_functions --no-fail-fast`: PASS, 1 test
- `PHP_SRC_DIR=$PHP_SRC_DIR REFERENCE_PHP=$REFERENCE_PHP PHPT_REUSE_LAST=0 PHPT_DEV_REUSE_TARGET_PASS=0 PHPT_TIMEOUT_SECONDS=10 nix develop -c just phpt-module-target MODULE=filesystem.streams FILE=ext/standard/tests/streams/stream_context_set_options_basic.phpt`: PASS, 1 target PHPT

Blocker report: no non-green outcomes remain in the promoted
`stream_context_set_options` basic upstream row. Broader context parameter
callbacks, stream filters, sockets, and user wrapper behavior remain outside
this selected gate.
