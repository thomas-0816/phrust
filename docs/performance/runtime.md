...
| `callgrind-smoke` | Optional Callgrind smoke; skips cleanly outside Linux, without `valgrind`, or when Valgrind cannot launch the VM runtime thread on the host, otherwise writes `target/performance/callgrind/summary.json`. |
