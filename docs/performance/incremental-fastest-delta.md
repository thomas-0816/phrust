...
| Persistent type feedback | present (guard-protected consumption) | `php_vm::persistent_feedback`; `--persistent-feedback-read/-write/-stats-json`; `FPE-GAP-PERSISTENT-FEEDBACK`; `docs/research/persistent-type-feedback.md`; optional matrix row `phrust-persistent-feedback-optional`. Consumption is governed by `--persistent-feedback-consume=off|quickening|quickening-ics` / `PHRUST_PERSISTENT_FEEDBACK_CONSUME` — **full consume-on by default** alongside the default sidecar (the guard protocol re-verifies every seed), attributed via `persistent_feedback_seeded_sites`/`_seeded_guard_hits`/`_seeded_dequickens` + `_seeded_callsites`/`_seeded_ic_hits`/`_seeded_ic_invalidations` and stats-JSON `consume_mode`. | (1) engine-owned **writer** with accept/reject-split stats, final-epoch capture, and function-callsite payload done (P1). (2) P3 done: `quickening-ics` (the default) seeds quickening **and** monomorphic entry-unit function-call ICs behind the full guard protocol. Method/property IC seeding stays blocked on replay-stable class identity. | P1 (done), P3 (done) |
| Include/autoload dependency graph | partial (positive reuse hardened) | Request-local projection over ICs; `FPE-GAP-INCLUDE-AUTOLOAD-GRAPH`; `docs/research/include-autoload-dependency-graph.md`. Directory-version-guarded negative caching remains active by default. Positive compiled reuse now uses stable opened-source generation plus exact-byte content identity for primaries and local dependencies; mutable hits validate content, while explicitly immutable hits require valid deployment, parent, and generation guards. Candidate paths are re-canonicalized to detect symlink swaps. | Function/constant graph nodes, cross-process positive reuse, and persistent-feedback integration remain future work. | P2 plus architecture remediation Prompt 01 |
...
1. **P1** — done: persistent-feedback writer + engine-owned metadata cache,
   and entries now carry the executed run's final invalidation epochs.
   Consumption is separately governed by `--persistent-feedback-consume`
   (consume-on default, kill switch).
...
3. **P3** — done: `quickening-ics` (default) seeds quickening and
   monomorphic function-call ICs with full attribution; method/property IC
   seeding stays blocked on replay-stable class identity.
