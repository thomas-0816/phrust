...
def main() -> int:
    ...
    print(f"[{report['status']}] call hotpath report wrote {rel(out_dir / 'wordpress-root.md')}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default=os.environ.get("PHRUST_CALL_HOTPATH_OUT", ""))
    parser.add_argument("--limit", type=int, default=int(os.environ.get("PHRUST_CALL_HOTPATH_LIMIT", "25")))
...
def summarize_profile(profile: dict[str, Any], limit: int) -> dict[str, Any]:
    ...
    return {
        ...
        "frame_activity": {
            ...
            "register_files_allocated": int_value(calls.get("register_files_allocated")),
            ...
            "direct_method_frame_hits": int_value(calls.get("direct_method_frame_hits")),
            "direct_closure_frame_hits": int_value(calls.get("direct_closure_frame_hits")),
            "direct_constructor_frame_hits": int_value(calls.get("direct_constructor_frame_hits")),
            "argument_vector_allocations_avoided": int_value(
                calls.get("argument_vector_allocations_avoided")
            ),
        },
        "top_calls": {
            "functions_by_inclusive_nanos": function_profiles[:limit],
            "methods_by_inclusive_nanos": method_profiles[:limit],
            "builtins_by_inclusive_nanos": builtin_profiles[:limit],
            ...
            "builtin_fast_stub_hits": list_entries(calls.get("builtin_fast_stub_hits"))[:limit],
            "builtin_fast_stub_misses": list_entries(calls.get("builtin_fast_stub_misses"))[:limit],
...
def self_test() -> int:
        ...
        "attribution": {
            "calls": {
                ...
                "function_profiles_by_name": [
                    {"name": "render", "count": 2, "inclusive_nanos": 200}
                ],
                "method_profiles_by_name": [
                    {"name": "Service::run", "count": 1, "inclusive_nanos": 100}
                ],
                "builtin_profiles_by_name": [
                    {"name": "count", "count": 3, "inclusive_nanos": 90}
                ],
                "dense_call_fallback_by_reason": [{"name": "dynamic", "value": 2}],
                "dense_function_fallback_by_reason": [{"name": "unsupported", "value": 1}],
                ...
                "builtin_fast_stub_fallback_by_reason": [{"name": "count.arity", "value": 1}],
                ...
                "call_frame_layout_observed": [{"name": "known_function_frame", "value": 4}],
                "generic_frame_fallback_by_reason": [{"name": "variadic", "value": 1}],
                "direct_frame_fallback_by_reason": [{"name": "dynamic_callable", "value": 1}],
            },
            "clones": {
                "value_clones": 22,
    ...
    assert summary["top_calls"]["functions_by_inclusive_nanos"][0]["name"] == "render"
