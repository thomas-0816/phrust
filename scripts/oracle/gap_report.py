...
def main() -> int:
    ...
    parser.add_argument("--cheap", action="store_true", help="skip PHPT and app-smoke target artifacts")
    ...
    try:
        ...
        entries = collect_entries(args.api, cheap=args.cheap)
...
def build_report(entries: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "entries": entries,
        "mode": "cheap" if all(entry["layer"] != "phpt" for entry in entries) else "full",
        "ratchet": {
