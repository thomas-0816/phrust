...
def main() -> int:
    ...
    print(f"[{report['status']}] wordpress root profile wrote {rel(out_dir / 'summary.md')}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--server", default=os.environ.get("PHRUST_SERVER", "target/debug/phrust-server"))
    parser.add_argument("--out", default="")
    parser.add_argument("--warmups", type=int, default=int(os.environ.get("PHRUST_WORDPRESS_PROFILE_WARMUPS", "1")))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.environ.get("PHRUST_WORDPRESS_PROFILE_TIMEOUT_SECONDS", "30")))
...
def output_dir(args: argparse.Namespace) -> Path:
    ...
    return REPO_ROOT / "target" / "performance" / "wordpress-root-profile" / now_run_id("root")
...
def run(args: argparse.Namespace, out_dir: Path) -> dict[str, Any]:
    ...
    if not profiles:
        return fail_report("server completed the request but wrote no request-profile JSON", args, out_dir, log_path, trace_path)
...
def request_root(address: str, timeout_seconds: float) -> dict[str, Any]:
    ...
    connection = http.client.HTTPConnection(host, int(port_text), timeout=timeout_seconds)
...
def summarize_profile(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "core_counters": {
            ...
            "vm_internal_function_dispatches": summary.get("vm_internal_function_dispatches", 0),
            "vm_include_rich_instructions_executed": summary.get("vm_include_rich_instructions_executed", 0),
            "vm_bytecode_instructions_executed": summary.get("vm_bytecode_instructions_executed", 0),
        },
        "top": {
            ...
            "include_fallback_by_reason": top_entries(includes.get("include_fallback_by_reason")),
            "dense_include_entry_fallback_by_reason": top_entries(includes.get("dense_include_entry_fallback_by_reason")),
            "builtin_fast_stub_fallback_by_reason": top_entries(calls.get("builtin_fast_stub_fallback_by_reason")),
            "array_fast_path_fallback_by_reason": top_entries(arrays.get("array_fast_path_fallback_by_reason")),
            "property_ic_fallback_reasons": top_entries(objects.get("property_ic_fallback_reasons")),
            "native_eligibility_rejections_by_reason": top_entries(native.get("native_eligibility_rejections_by_reason")),
        },
    }
...
def self_test() -> int:
    profile = {
        ...
        "attribution": {
            ...
            "includes": {"include_fallback_by_reason": [{"name": "unsupported", "count": 1}]},
            "calls": {},
            "arrays": {},
            "objects": {},
            "native": {},
    ...
    assert summary["top"]["array_handle_clone_by_source_family"][0]["name"] == "array_element_read"
    assert summary["top"]["cow_separation_by_source_family"][0]["name"] == "array_element_write"
    assert (
