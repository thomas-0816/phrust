...
export async function setOutgoingResponse(id, { statusCode, headers, streamId }) {
...
export async function startHttpServer(id, { port, host }) {
    ...
    await new Promise((resolve, reject) => {
        // @ts-expect-error Resolve has different signature
        server.listen(port, host, resolve);
        server.on("error", reject);
    });
    servers.set(id, server);
}
export async function createHttpRequest(method, scheme, authority, pathWithQuery, headers, bodyId, connectTimeout, betweenBytesTimeout, firstByteTimeout) {
    let stream = null;
    if (bodyId) {
        try {
    ...
    try {
        ...
        if (firstByteTimeout) {
            res.setTimeout(Number(firstByteTimeout / 1000000n));
        }
