...
let pollCnt = 0, streamCnt = 0, futureCnt = 0;
...
function handle(call, id, payload) {
    ...
    switch (call) {
        ...
        case HTTP_CREATE_REQUEST: {
            ...
            return createFuture(createHttpRequest(method, scheme, authority, pathWithQuery, headers, body, connectTimeout, betweenBytesTimeout, firstByteTimeout), undefined);
        }
        ...
        case OUTPUT_STREAM_DISPOSE | HTTP:
            throw new Error("wasi-io trap: Output stream dispose not implemented as an IO-call for HTTP");
        ...
        case SOCKET_RESOLVE_ADDRESS_CREATE_REQUEST:
            return createFuture(socketResolveAddress(payload), undefined);
        ...
        case SOCKET_GET_DEFAULT_SEND_BUFFER_SIZE:
            return getDefaultSendBufferSize();
        case SOCKET_GET_DEFAULT_RECEIVE_BUFFER_SIZE:
            return getDefaultReceiveBufferSize();
        case SOCKET_TCP_SET_LISTEN_BACKLOG_SIZE:
            return socketTcpSetListenBacklogSize(id, undefined);
        ...
        case INPUT_STREAM_CREATE | STDIN: {
            return createReadableStream(new Readable({
                read(n) {
                    if (n <= 0) {
    ...
    switch (call & CALL_MASK) {
        ...
        case INPUT_STREAM_SKIP:
            return handle(INPUT_STREAM_READ | (call & CALL_TYPE_MASK), id, new Uint8Array(Number(payload)));
        case INPUT_STREAM_BLOCKING_SKIP:
            return handle(INPUT_STREAM_BLOCKING_READ | (call & CALL_TYPE_MASK), id, new Uint8Array(Number(payload)));
        ...
        case OUTPUT_STREAM_BLOCKING_WRITE_AND_FLUSH: {
            ...
            if (payload.byteLength > stream.stream.writableHighWaterMark - stream.stream.writableLength) {
        ...
        case OUTPUT_STREAM_BLOCKING_FLUSH: {
            ...
            return new Promise((resolve, reject) => {
                if (stream.stream === stdout || stream.stream === stderr) {
                    ...
                    // For all other writes, we can perform the actual write and expect the write to complete
                    // and trigger the relevant callback
                    stream.stream.write(new Uint8Array([]), (err) => err ? reject(streamError(err)) : resolve());
                }
            });
        }
        ...
        case OUTPUT_STREAM_SPLICE: {
            ...
            while (bytesRemaining > 0 &&
                (chunk = inputStream.stream.read(Math.min(outputStream.writableHighWaterMark - outputStream.writableLength, bytesRemaining)))) {
        ...
        case OUTPUT_STREAM_BLOCKING_SPLICE: {
            ...
            if (!inputStream.stream.readable) {
                promise = promise.then(() => new Promise((_resolve, _reject) => {
                    inputStream.stream
        ...
        case POLL_POLLABLE_DISPOSE:
            if (!polls.delete(id)) {
                throw new Error(`wasi-io trap: Disposed a poll ${id} that does not exist`);
            }
            return;
        ...
        case FUTURE_DISPOSE:
            return void futureDispose(id, true);
        default:
            throw new Error(`wasi-io trap: Unknown call ${call} (${reverseMap[call]}) with type ${reverseMap[call & CALL_TYPE_MASK]}`);
    }
}
/**
...
export function verifyPollsDroppedForDrop(pollState, polledResourceDebugName) {
    for (const pollId of pollState.polls) {
        ...
        if (poll) {
            throw new Error(`wasi-io trap: Cannot drop ${polledResourceDebugName} as it has a child poll resource which has not yet been dropped`);
        }
    }
}
/**
...
export function pollStateReady(pollState) {
    if (pollState.ready && pollState.listener) {
        uncaughtException = new Error("wasi-io trap: poll already ready with listener attached");
    }
