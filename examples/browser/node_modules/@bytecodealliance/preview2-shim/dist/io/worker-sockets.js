...
export function isMulticastIpAddress(ipSocketAddress) {
    return ((ipSocketAddress.tag === "ipv4" && ipSocketAddress.val.address[0] === 0xe0) ||
...
export function isBroadcastIpAddress(ipSocketAddress) {
    const { address } = ipSocketAddress.val;
    return (ipSocketAddress.tag === "ipv4" &&
        address[0] === 0xff &&
        address[1] === 0xff &&
        address[2] === 0xff &&
        address[3] === 0xff);
...
export function ipv6ToTuple(ipv6) {
    ...
    return [...lhsParts, ...Array(8 - lhsParts.length - rhsParts.length).fill(0), ...rhsParts].map((segment) => parseInt(segment, 16));
}
