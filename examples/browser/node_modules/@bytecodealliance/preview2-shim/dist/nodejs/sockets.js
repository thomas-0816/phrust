import { SOCKET_DATAGRAM_STREAM_DISPOSE, SOCKET_DATAGRAM_STREAM_SUBSCRIBE, SOCKET_INCOMING_DATAGRAM_STREAM_RECEIVE, SOCKET_OUTGOING_DATAGRAM_STREAM_CHECK_SEND, SOCKET_OUTGOING_DATAGRAM_STREAM_SEND, SOCKET_RESOLVE_ADDRESS_CREATE_REQUEST, SOCKET_RESOLVE_ADDRESS_DISPOSE_REQUEST, SOCKET_RESOLVE_ADDRESS_SUBSCRIBE_REQUEST, SOCKET_RESOLVE_ADDRESS_TAKE_REQUEST, SOCKET_GET_DEFAULT_RECEIVE_BUFFER_SIZE, SOCKET_GET_DEFAULT_SEND_BUFFER_SIZE, SOCKET_TCP_ACCEPT, SOCKET_TCP_BIND_FINISH, SOCKET_TCP_BIND_START, SOCKET_TCP_CONNECT_FINISH, SOCKET_TCP_CONNECT_START, SOCKET_TCP_CREATE_HANDLE, SOCKET_TCP_DISPOSE, SOCKET_TCP_GET_LOCAL_ADDRESS, SOCKET_TCP_GET_REMOTE_ADDRESS, SOCKET_TCP_IS_LISTENING, SOCKET_TCP_LISTEN_FINISH, SOCKET_TCP_LISTEN_START, SOCKET_TCP_SET_KEEP_ALIVE, SOCKET_TCP_SET_LISTEN_BACKLOG_SIZE, SOCKET_TCP_SHUTDOWN, SOCKET_TCP_SUBSCRIBE, SOCKET_TCP, SOCKET_UDP_BIND_FINISH, SOCKET_UDP_BIND_START, SOCKET_UDP_CREATE_HANDLE, SOCKET_UDP_DISPOSE, SOCKET_UDP_GET_LOCAL_ADDRESS, SOCKET_UDP_GET_RECEIVE_BUFFER_SIZE, SOCKET_UDP_GET_REMOTE_ADDRESS, SOCKET_UDP_GET_SEND_BUFFER_SIZE, SOCKET_UDP_GET_UNICAST_HOP_LIMIT, SOCKET_UDP_SET_RECEIVE_BUFFER_SIZE, SOCKET_UDP_SET_SEND_BUFFER_SIZE, SOCKET_UDP_SET_UNICAST_HOP_LIMIT, SOCKET_UDP_STREAM, SOCKET_UDP_SUBSCRIBE, } from "../io/calls.js";
import { earlyDispose, inputStreamCreate, ioCall, outputStreamCreate, pollableCreate, registerDispose, } from "../io/worker-io.js";
...
class ResolveAddressStream {
    ...
    resolveNextAddress() {
        ...
        if (this.#data && this.#curItem < this.#data.length) {
            return this.#data[this.#curItem++];
        }
        return undefined;
    }
    subscribe() {
        return pollableCreate(ioCall(SOCKET_RESOLVE_ADDRESS_SUBSCRIBE_REQUEST, this.#id, null), this);
    }
    static _resolveAddresses(network, name) {
        if (!(network instanceof Network) || !mayDnsLookup(network)) {
        ...
        res.#finalizer = registerDispose(res, null, res.#id, resolveAddressStreamDispose);
        return res;
    }
    [symbolDispose]() {
        if (this.#finalizer) {
...
class TcpSocket {
    ...
    #options = {
        ...
        // instead.
        keepAliveIdleTime: 7200000000000n,
    ...
    finishConnect() {
        const [inputStreamId, outputStreamId] = ioCall(SOCKET_TCP_CONNECT_FINISH, this.#id, null);
        return [
    ...
    accept() {
        ...
        const [socketId, inputStreamId, outputStreamId] = ioCall(SOCKET_TCP_ACCEPT, this.#id, null);
...
export const tcpCreateSocket = {
    createTcpSocket(addressFamily) {
        ...
        return tcpSocketCreate(addressFamily, ioCall(SOCKET_TCP_CREATE_HANDLE, null, null));
    },
};
export const tcp = {
    TcpSocket,
...
class UdpSocket {
    ...
    static _create(addressFamily) {
        ...
        socket.#family = addressFamily;
        socket.#finalizer = registerDispose(socket, null, socket.#id, socketUdpDispose);
        return socket;
    }
    startBind(network, localAddress) {
        if (!mayUdp(network)) {
    ...
    stream(remoteAddress) {
        ...
        const [incomingDatagramStreamId, outgoingDatagramStreamId] = ioCall(SOCKET_UDP_STREAM, this.#id, remoteAddress);
        return [
...
class IncomingDatagramStream {
    ...
    static _create(id) {
        ...
        stream.#id = id;
        stream.#finalizer = registerDispose(stream, null, id, incomingDatagramStreamDispose);
        return stream;
    }
    receive(maxResults) {
        return ioCall(SOCKET_INCOMING_DATAGRAM_STREAM_RECEIVE, this.#id, maxResults);
    }
    subscribe() {
        return pollableCreate(ioCall(SOCKET_DATAGRAM_STREAM_SUBSCRIBE, this.#id, null), this);
    }
    [symbolDispose]() {
        if (this.#finalizer) {
            earlyDispose(this.#finalizer);
...
class OutgoingDatagramStream {
    ...
    static _create(id) {
        ...
        stream.#id = id;
        stream.#finalizer = registerDispose(stream, null, id, outgoingDatagramStreamDispose);
        return stream;
    }
    checkSend() {
