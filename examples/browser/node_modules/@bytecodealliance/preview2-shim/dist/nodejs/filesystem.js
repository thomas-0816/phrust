import { earlyDispose, inputStreamCreate, ioCall, outputStreamCreate, registerDispose, } from "../io/worker-io.js";
import { FILE, INPUT_STREAM_CREATE, OUTPUT_STREAM_CREATE } from "../io/calls.js";
import { closeSync, constants, fdatasyncSync, fstatSync, fsyncSync, ftruncateSync, futimesSync, linkSync, lstatSync, lutimesSync, mkdirSync, opendirSync, openSync, readlinkSync, readSync, renameSync, rmdirSync, statSync, symlinkSync, unlinkSync, utimesSync, writeSync, } from "node:fs";
import { platform } from "node:process";
const symbolDispose = Symbol.dispose || Symbol.for("dispose");
const isWindows = platform === "win32";
const isMac = platform === "darwin";
const nsMagnitude = 1000000000n;
...
class Descriptor {
    ...
    readViaStream(offset) {
        ...
        return inputStreamCreate(FILE, ioCall(INPUT_STREAM_CREATE | FILE, null, {
            fd: this.#fd,
            offset,
        }));
    }
    writeViaStream(offset) {
        ...
        return outputStreamCreate(FILE, ioCall(OUTPUT_STREAM_CREATE | FILE, null, { fd: this.#fd, offset }));
    }
    appendViaStream() {
    ...
    setTimes(dataAccessTimestamp, dataModificationTimestamp) {
        ...
        const atime = this.#getNewTimestamp(dataAccessTimestamp, dataAccessTimestamp.tag === "no-change" && stats.dataAccessTimestamp);
        const mtime = this.#getNewTimestamp(dataModificationTimestamp, dataModificationTimestamp.tag === "no-change" && stats.dataModificationTimestamp);
        try {
...
class DirectoryEntryStream {
    ...
    static _create(dir) {
        ...
        dirStream.#finalizer = registerDispose(dirStream, null, 0, dir.closeSync.bind(dir));
        dirStream.#dir = dir;
        return dirStream;
    }
    [symbolDispose]() {
...
function timestampToMs(timestamp) {
    ...
    if (secondsInMillis > Number.MAX_SAFE_INTEGER) {
        throw new Error(`cannot represent millisecond amount as Number [${{ secondsInMillis }}]`);
    }
