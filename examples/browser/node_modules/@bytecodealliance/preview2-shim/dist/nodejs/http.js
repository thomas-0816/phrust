import { FUTURE_DISPOSE, FUTURE_SUBSCRIBE, FUTURE_TAKE_VALUE, HTTP_CREATE_REQUEST, HTTP_OUTGOING_BODY_DISPOSE, HTTP_OUTPUT_STREAM_FINISH, HTTP_SERVER_CLEAR_OUTGOING_RESPONSE, HTTP_SERVER_SET_OUTGOING_RESPONSE, HTTP_SERVER_START, HTTP_SERVER_STOP, HTTP_SERVER_GET_ADDRESS, OUTPUT_STREAM_CREATE, OUTPUT_STREAM_DISPOSE, } from "../io/calls.js";
import { earlyDispose, inputStreamCreate, ioCall, outputStreamCreate, pollableCreate, registerDispose, registerIncomingHttpHandler, } from "../io/worker-io.js";
...
const defaultHttpTimeout = 600000000000n;
...
class OutgoingRequest {
    ...
    static _handle(request, options) {
        ...
        return futureIncomingResponseCreate(request.#method.val || request.#method.tag, scheme, request.#authority, request.#pathWithQuery, headers, outgoingBodyOutputStreamId(request.#body), connectTimeout, betweenBytesTimeout, firstByteTimeout);
    }
}
...
class OutgoingBody {
    ...
    static _create(contentLength) {
        ...
        outgoingBody.#outputStreamId = ioCall(OUTPUT_STREAM_CREATE | HTTP, null, outgoingBody.#contentLength);
        outgoingBody.#outputStream = outputStreamCreate(HTTP, outgoingBody.#outputStreamId);
        outgoingBody.#finalizer = registerDispose(outgoingBody, null, outgoingBody.#outputStreamId, outgoingBodyDispose);
        return outgoingBody;
    }
    [symbolDispose]() {
        if (this.#finalizer) {
...
class FutureIncomingResponse {
    ...
    #finalizer;
    subscribe() {
        return pollableCreate(ioCall(FUTURE_SUBSCRIBE | HTTP, this.#id, null), this);
    }
    get() {
        ...
        if (ret.tag === "ok" && ret.val.tag === "ok") {
            ...
            ret.val.val = incomingResponseCreate(status, fieldsFromEntriesChecked(headers.map(([key, val]) => [key, textEncoder.encode(val)])), bodyStreamId);
        }
        return ret;
    }
    static _create(method, scheme, authority, pathWithQuery, headers, body, connectTimeout, betweenBytesTimeout, firstByteTimeout) {
        ...
        res.#finalizer = registerDispose(res, null, res.#id, futureIncomingResponseDispose);
        return res;
    }
    [symbolDispose]() {
        if (this.#finalizer) {
...
export class HTTPServer {
    ...
    constructor(incomingHandler) {
        ...
        registerIncomingHttpHandler(this.#id, ({ method, pathWithQuery, host, headers, responseId, streamId }) => {
            const textEncoder = new TextEncoder();
            const request = incomingRequestCreate(parseMethod(method), pathWithQuery, { tag: "HTTP" }, host, fieldsLock(fieldsFromEntriesChecked(headers
                .filter(([key]) => !_forbiddenHeaders.has(key))
                .map(([key, val]) => [key, textEncoder.encode(val)]))), streamId);
