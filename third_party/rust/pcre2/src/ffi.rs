...
use std::{cmp, ffi::CStr, ptr, slice};
...
pub(crate) struct MatchConfig {
    ...
    /// When set, limits PCRE2's match/backtracking work.
    pub(crate) match_limit: Option<u32>,
    /// When set, limits PCRE2's nested match depth.
...
impl MatchData {
    ...
    pub(crate) fn new(config: MatchConfig, code: &Code) -> MatchData {
        ...
        if let Some(limit) = config.match_limit {
            let rc = unsafe { pcre2_set_match_limit_8(match_context, limit) };
            assert_eq!(rc, 0, "failed to set PCRE2 match limit");
        }
        if let Some(limit) = config.depth_limit {
            let rc = unsafe { pcre2_set_depth_limit_8(match_context, limit) };
            assert_eq!(rc, 0, "failed to set PCRE2 depth limit");
        }
    ...
    /// Return the last PCRE2 mark reached by the match, if any.
    pub(crate) fn mark(&self) -> Option<&[u8]> {
        let mark = unsafe { pcre2_get_mark_8(self.match_data) };
        if mark.is_null() {
            return None;
        }
        Some(unsafe { CStr::from_ptr(mark.cast()).to_bytes() })
    }
}
