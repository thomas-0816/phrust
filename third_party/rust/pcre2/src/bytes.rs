...
use pcre2_sys::{
    PCRE2_ANCHORED, PCRE2_CASELESS, PCRE2_DOTALL, PCRE2_EXTENDED,
    PCRE2_MATCH_INVALID_UTF, PCRE2_MULTILINE, PCRE2_NEWLINE_ANYCRLF,
    PCRE2_NOTEMPTY_ATSTART, PCRE2_UCP, PCRE2_UNSET, PCRE2_UTF,
};
...
/// Match-time options exposed for callers that need PCRE2 iteration parity.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct MatchOptions {
    anchored: bool,
    not_empty_at_start: bool,
}

impl MatchOptions {
    /// Require a match to start at the requested offset.
    #[must_use]
    pub const fn anchored(mut self, enabled: bool) -> Self {
        self.anchored = enabled;
        self
    }

    /// Reject empty matches at the requested offset.
    #[must_use]
    pub const fn not_empty_at_start(mut self, enabled: bool) -> Self {
        self.not_empty_at_start = enabled;
        self
    }
    ...
    const fn bits(self) -> u32 {
        ...
        if self.not_empty_at_start {
            bits |= PCRE2_NOTEMPTY_ATSTART;
        }
        bits
    }
}
...
impl RegexBuilder {
    ...
    /// Sets PCRE2's match/backtracking work limit.
    ///
    /// When `None`, PCRE2 uses its library default.
    pub fn match_limit(&mut self, limit: Option<u32>) -> &mut RegexBuilder {
        self.config.match_config.match_limit = limit;
        self
    }

    /// Sets PCRE2's nested match depth limit.
    ///
    /// When `None`, PCRE2 uses its library default.
    pub fn depth_limit(&mut self, limit: Option<u32>) -> &mut RegexBuilder {
        self.config.match_config.depth_limit = limit;
        self
    }
}
...
impl Regex {
    ...
    /// Returns the first set of captures found at or after `start`.
    pub fn captures_at<'s>(
        &self,
        subject: &'s [u8],
        start: usize,
    ) -> Result<Option<Captures<'s>>, Error> {
        self.captures_at_with_options(subject, start, MatchOptions::default())
    }

    /// Returns captures found at or after `start` with PCRE2 match options.
    pub fn captures_at_with_options<'s>(
        ...
        let mut locs = self.capture_locations();
        Ok(self
            .captures_read_at_with_options(&mut locs, subject, start, options)?
            .map(move |_| Captures {
                subject,
                locs,
                idx: Arc::clone(&self.capture_names_idx),
...
impl Regex {
    ...
    pub fn captures_read_at<'s>(
        ...
        self.captures_read_at_with_options(
            locs,
            subject,
            start,
    ...
    /// Returns the same as `captures_read_at`, with PCRE2 match options.
    pub fn captures_read_at_with_options<'s>(
        &self,
        locs: &mut CaptureLocations,
        subject: &'s [u8],
        start: usize,
        ...
        // SAFETY: MatchOptions only exposes regular PCRE2 match flags.
        if unsafe { !locs.data.find(&self.code, subject, start, options.bits())? } {
            return Ok(None);
        }
...
impl<'s> Captures<'s> {
    ...
    /// Returns the last PCRE2 mark reached by this match, if any.
    pub fn mark(&self) -> Option<&[u8]> {
        self.locs.data.mark()
    }
